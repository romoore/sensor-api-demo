...
public class DataStore {
  ...
  private static final class RSSIDateComparator implements Comparator<RSSIDatum>{
  ...
  private final ConcurrentSkipListSet<RSSIDatum> rssiQueue = new ConcurrentSkipListSet<RSSIDatum>(new RSSIDateComparator());
//  private final ConcurrentLinkedQueue<RSSIDatum> rssiQueue = new ConcurrentLinkedQueue<>(); 
  ...
  public void addSample(SampleMessage sample){
    RSSIDatum datum = new RSSIDatum(toHexString(sample.getDeviceId()), toHexString(sample.getReceiverId()), sample.getRssi(), sample.getCreationTimestamp());
